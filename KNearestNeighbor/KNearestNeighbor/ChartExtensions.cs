...
namespace KNearestNeighbor
{
    ...
    {
        public static void PlotPoints(Chart chart, int xCoordChoice, int yCoordChoice, List<List<double>> normalizedTrainingSet, List<int> 
            outputClass, List<double> inputSet)
        {
        ...
        public static void PlotPoints(Chart chart, int xCoordChoice, int yCoordChoice, List<List<double>> trainingSet, List<List<double>> normalizedTrainingSet, List<int> outputs, List<double> inputData)
        {
            int point = 0;
            for (int index = 0; index < outputs.Count; index++)
            {
                for (int count = 0; count < trainingSet.Count; count++)
                {
                    ...
                    //Need to plot by series NUMBER which is equal to the outputClass.
                    chart.Series[index].Points.AddXY(normalizedTrainingSet[count][xCoordChoice], normalizedTrainingSet[count][yCoordChoice]);
